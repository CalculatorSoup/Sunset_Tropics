...
namespace Tropics.Content
{
    ...
    {
        public string identifier => Tropics.GUID + "." + nameof(ContentProvider);
        ...
        public IEnumerator LoadStaticContentAsync(LoadStaticContentAsyncArgs args)
        {
            ...
            yield return LoadAssetBundle(
                Path.Combine(assetsFolderFullPath, TropicsContent.ScenesAssetBundleFileName),
                args.progressReceiver,
                (assetBundle) => scenesAssetBundle = assetBundle);

            AssetBundle assetsAssetBundle = null;
            yield return LoadAssetBundle(
                Path.Combine(assetsFolderFullPath, TropicsContent.AssetsAssetBundleFileName),
                args.progressReceiver,
                (assetBundle) => assetsAssetBundle = assetBundle);

            yield return TropicsContent.LoadAssetBundlesAsync(
